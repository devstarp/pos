...
// } from '../config/api';
import { getGeneralErrorMessages, get400ErrorMessages, get422ErrorObject, get430ErrorObject } from './errorUtils';
...
export const query = async function (path, options = {}, useToken = true, skipNotify = true, skipNetworkStatus = false) {
  ...
  try {
    ...
    if (errorResponse === undefined || !errorResponse) {
      ...
      errorMessage = errorResponse.data.message || 'Failed by server error.';
      // if (isDevEnv) { message = 'Failed by server error.'; }
    } else if (errorResponse.status === 422) {
      errorMessage = get422ErrorObject(errorResponse.data.data);
    } else if (errorResponse.status === 430) {
      errorMessage = get430ErrorObject(errorResponse.data.data);
    } else if (errorResponse.status === 409) {
      errorMessage = errorResponse.data.data;
      ...
      staticStore && staticStore.dispatch(clearAuthState());
      removeLocalToken();
      // history.push('/login');
    } else {
      status = errorResponse.status;
