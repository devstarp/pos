...
class StockPage extends Component<Props> {
  ...
  render() {
    ...
    const menu = (
      <Menu>
        <MenuItem>
          <Button onClick={() => this.handleSetFavorite()} style={{ border: '0', background: 'transparent' }}><Icon
            type="star" 
            theme={this.state.selected ? (this.state.selected.isFavorite ? "filled" : "outlined") : "outlined"} 
          />{lang.favorite}
          </Button>
        </MenuItem>
        <MenuItem>
          ...
          <Form className='stock-form'>
            ...
            >
              {getFieldDecorator('barkod', {
                ...
                >{this.props.products.map(p => <Option key={p.id} a={p} id={p.barcode}>{p.name}</Option>)}
                </Select>
              )}
              ...
              <div>
                ...
                >
                  {getFieldDecorator('dealer', {
                    ...
                    >{this.props.dealers.map(p => <Option key={p.id}>{p.name}</Option>)}
                    </Select>
                  )}
                </FormItem>
                <FormItem
                ...
                >
                  {getFieldDecorator('alisFiyati', {
                    ...
                    <InputNumber min={0} formatter={value => `${value + lang.currency}`} disabled />
                  )}
                </FormItem>
                <FormItem
...
export { ConnectedPage as StockPage }
