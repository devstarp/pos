...
import { Button, Row, Col, Select, Form, Popconfirm, Statistic, InputNumber, Space, Modal } from 'antd'
import { connect } from 'react-redux';
import PropTypes from 'prop-types';
import { setProducts, addProduct} from 'redux/slices/product'
import {DeleteOutlined, EyeOutlined} from '@ant-design/icons'
import { apiCreateProduct, apiGetProducts } from 'api/product';
import CreateProductModal from 'components/Modal/CreateProductModal'
import { findIndex, filter } from 'lodash';
import EditableTable from 'components/Table/EditableTable';
import { setCategories } from 'redux/slices/category';
import { apiGetCategories } from 'api/category';
import { apiCreatePurchaseWithItems } from 'api/purchase';
...
const NewPurchasePage =({dispatch, product, category, history, supplier})=>{ 
  ...
  const [selectedFields, setSelectedFields]=useState(['name', 'category_id', 'purchase_price', 'purchase_currency'])
  ...
  const handleSelectProduct= (_product, selected) => {
    if(selected){
      const tmpProduct = { 
        currency:_product.purchase_currency, 
        quantity:1, 
        price:Number(_product.purchase_price), 
  ...
  const handleCreatePurchaseWithItems =async()=>{
    try {
      const draft = await form.validateFields();
      const resPurchase = await apiCreatePurchaseWithItems(draft);
      if(resPurchase&&!resPurchase.error){
        history.push('/success', {state:{order:resPurchase.data, type:'PURCHASE'}})
      }else{
        resPurchase.error && form .setFields(resPurchase.error)
      }
    } catch (errInfo) {
  ...
  const handleCreateProduct=async(_product)=>{
    const resProduct = await apiCreateProduct(_product)
    if(resProduct&&!resProduct.error){
      handleSelectProduct(resProduct.data,true)
    ...
    <Row gutter={[5, 20]}>
      <Col xs={24} md={13}>
        <Form form={form} initialValues={{items:[], currency:'USD'}} component={false}>
          <Form.Item name='supplier_id' label='Supplier' rules={[{required:true, message:'Please select the supplier'}]}>
            <APISearchSelect
              defaultOptions={supplier.suppliers}
              debounceTimeout={100}
              apiGetOptions={apiGetSuppliers}
              fieldNames={{label:'name', value:'id'}}
              createFieldName="name"
              apiCreateOption={apiCreateSupplier}
            />
          </Form.Item>
          ...
          <Popconfirm  title="Sure to purchase?" onConfirm={handleCreatePurchaseWithItems} >
