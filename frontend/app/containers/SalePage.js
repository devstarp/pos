...
class SalePage extends Component<Props> {
    ...
    calculateTotal = (e) => {
      ...
      this.state.products.map(product =>
        total += product.product.salePrice * this.state.quantities[product.id]
      )
      return total;
    }
    ...
    render() {
      ...
      this.props.stocks&&this.props.stocks.map(i => breadcrumbNameMap[`/sale/${i.product.category}`] = i.product.category)
                ...
                {this.state.stocks.map((stock, index) => (
                  <div className='sale-products' onClick={() => this.handleRightItemClick(index)}>
                    ...
                    <div style={{ backgroundColor: 'white', color: '#383c43', margin: '-15px', display: 'flex', alignItems: 'center', flexDirection: 'column', justifyContent: 'center', height: '50px' }}>
                      ...
                      <div className='ribbon'><div className='shadow' />{stock.product.salePrice}{lang.currency}<span className='ribbon-a' /></div>
                    </div>
              ...
              {/* <Form className='stock-form'>
                ...
                >
                  {getFieldDecorator('personId', {
                    ...
                    <Select
                      placeholder={lang.choosePerson}
                      style={{ width: '200px' }}
                      showSearch
                      filterOption={(input, option) => (option.props.children).toLowerCase().indexOf(input.toLowerCase()) >= 0}
                    >{this.props.clients&&this.props.clients.map(p => <Option key={p.id}>{p.name}</Option>)}
                    </Select>
                  )}
                </FormItem>
...
// const WrappedPage = Form.create()(SalePage);
