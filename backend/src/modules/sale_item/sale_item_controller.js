...
const DEFAULT_INCLUDES='customer,author,sale,product';
...
const create = async (req, res) => {
  try {
    ...
    const sale = await createSaleItem({...req.body, author_id:employee_id}, req.query.includes || DEFAULT_INCLUDES);
...
const update = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const { author_id } = req.app.locals;
