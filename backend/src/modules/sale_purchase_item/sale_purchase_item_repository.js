...
const { Sales, SaleItems, Products, Customers, Users, SalePurchaseItems } = db;
...
export const getWhere=(query)=>{
  ...
  //user_id, category_id, sale_currency, sale_currency, barcode, 
  if(query.id){
    ...
    switch (include) {
      ...
      case 'sale':
        result.push({
          model:Sales,
          as: 'sale',
          attributes: ['id', 'order_number', 'user_id'],
        })
        break;
      default:
        break;
...
const findListSaleItem = async (query,  page, limit, includes, paranoid) => {
  try {
    const result = await SalePurchaseItems.findAndCountAll({
      where:getWhere(query), 
      include:getInclude(includes),
...
const createSalePurchaseItem = async (data, includes, transaction) => {
  const t = transaction ? transaction : await db.sequelize.transaction();
  try {
    let result = await SalePurchaseItems.create(data, { transaction });
    if(includes){
    ...
    console.error('[EXCEPTION] create sale item', error);
    throw new Error(error);
  }
};
...
const deleteSaleItem = async (query, force, transaction) => {
  ...
  try {
    ...
    if (!transaction) t.rollback();
    console.error('[EXCEPTION] deleteSaleItem', error);
    throw new Error(error);
  }
};
...
const restoreSaleItem = async (query, includes, transaction) => {
  ...
  try {
    ...
    if (!transaction) t.rollback();
    console.error('[EXCEPTION] restoreSaleItem', error);
    throw error;
  }
};
...
export { 
  ...
  createSalePurchaseItem, updateSaleItem, findOneSaleItem, restoreSaleItem 
};
